...
ell(int(1.28*W),int(-0.12*H),int(0.44*W),accent,58)
ell(int(-0.08*W),int(1.10*H),int(0.30*W),(236,220,240),55)
blob=blob.filter(ImageFilter.GaussianBlur(120*S//3))
...
wins=[(1068,44,214,150,'sky','blue'),
      (1332,120,196,152,'peri','purple'),
      (1150,236,198,138,'mint','mint')]
...
for x,y,w,h,ic,stc in wins:
    ...
    for i,c in enumerate(dots):
        dx=x+16*S+i*16*S; draw.ellipse([dx-dr,dy-dr,dx+dr,dy+dr],fill=c)
    m=16*S; ib=[x+m,y+th+10*S,x+w-m,y+h-m]
    draw.rounded_rectangle(ib,radius=8*S,fill=inners[ic])
    scx=(ib[0]+ib[2])/2; scy=(ib[1]+ib[3])/2
    sparkle(scx,scy,min(ib[2]-ib[0],ib[3]-ib[1])*0.5,sk[stc])

# sparkles clear of text box x[400,950] y[80,300]
for cx,cy,sz,c in [(520,335,22,'coral'),(690,346,26,'mint'),(880,330,20,'blue'),
                   (985,62,18,'purple'),(1002,96,22,'coral'),(1010,326,28,'yellow'),
                   (1300,300,22,'pink'),(1560,120,20,'purple'),(1548,332,26,'blue')]:
    sparkle(cx*S,cy*S,sz*S,sk[c])

FS='/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf'
FM='/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf'
f_name=ImageFont.truetype(FS,76*S)
f_lab=ImageFont.truetype(FM,26*S)
f_sub=ImageFont.truetype(FM,22*S)
X0=404*S
def track(txt,font,x,y,fill,sp):
    for ch in txt:
        draw.text((x,y),ch,font=font,fill=fill); x+=draw.textlength(ch,font=font)+sp*S

draw.text((X0,92*S),"Christine Li",font=f_name,fill=ink)
track("DATA · AI PRODUCT · ML",f_lab,X0,192*S,accent,2)
draw.text((X0,234*S),"M. Analytics · UC Berkeley",font=f_sub,fill=ink_soft)
draw.text((X0,268*S),"B.S. Statistics · UC Davis",font=f_sub,fill=ink_soft)

img.resize((1584,396),Image.LANCZOS).save('linkedin_banner.png')
print("saved")
